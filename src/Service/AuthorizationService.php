...
class AuthorizationService
{
    ...
    public function checkPermission(int $userId, string $permissionName, ?int $targetId = null): bool
    {
        ...
        if ($userPermission && !$userPermission->isDenied()) {
            return true;
        }

        // Kiểm tra quyền của nhóm
        $groupIds = $this->groupMemberRepository->findGroupIdsByUserId($userId);
        $groupPermission = $this->groupPermissionRepository->findOneBy([
            'group_id' => $groupIds,
            'permission_id' => $permissionName,
            'target_id' => $targetId,
            'is_active' => true,
        ...
        if ($groupPermission && !$groupPermission->isDenied()) {
            return true;
        }
